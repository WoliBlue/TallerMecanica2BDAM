...
public class DerrapLogin extends JFrame {
    ...
    public DerrapLogin() {
        ...
        try {
            ...
            InputStream fontStream = getClass().getResourceAsStream("/com/derrap/fonts/MartianMono-Variable.ttf");
            if (fontStream == null) {
                throw new RuntimeException("Font not found!");
            }
            Font martianMono = Font.createFont(Font.TRUETYPE_FONT, fontStream).deriveFont(16f);
            fontStream.close();

            // Load the second font
            InputStream outfitStream = getClass().getResourceAsStream("/com/derrap/fonts/Outfit-SemiBold.ttf");
            if (outfitStream == null) {
                throw new RuntimeException("Font not found!");
            }
            Font outfit = Font.createFont(Font.TRUETYPE_FONT, outfitStream).deriveFont(32f);
            outfitStream.close();
        ...
        // Logo
        JLabel logo = new JLabel(new ImageIcon(getClass().getResource("/com/derrap/resources/icon2.png")));
